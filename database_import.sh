...
DATABASE_URL="10.9.62.89"
CREDENTIALS_URL="10.9.62.89"
...
sudo wget http://$CREDENTIALS_URL/images/credentials.txt -O /home/vagrant/credentials.txt
if [ $? -ne 0 ]; then
...
`su - postgres -c "pg_restore -e -U root -w -d vmdb_production /repo/vmdb_production_latest.dump"`
if [ $? -ne 0 ]; then
...
sudo su - root -c "rm /repo/vmdb_production_latest.dump"
